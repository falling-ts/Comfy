...
| **`workflows`** | **用户工作流实际存储处**(前端保存即在此,可经 `GET /userdata?dir=workflows` 读取),共 25 个,按编号-用途分组: |
| └ `1xxx` 万物 | `1000-万物建模`(主线主流程)/ `1001-灰度遮罩` / `1010-万物变化` |
| └ `2xxx` 场景镜头 | `2000-场景首帧` / `2010-场景拉镜` / `2020-场景推镜` / `2030-场景旋镜` |
| └ `3xxx` 场景生成 | `3000-文生场景`(H3 T2VA,仅画面)/ `3010-首帧场景`(I2V)/ `3020-参考场景`(R2V 多图多视频参考)/ `3030-OrbitSheets场景`(Location Sheet 参考板:锚点图+H3 多视角选帧拼板)/ `3040-Skythread场景`(R2V 三参考法:角色/道具/空场景) |
| └ `4xxx` 视频生成 | `4000-文生视频` / `4010-首帧视频` / `4020-首尾视频` / `4025-关键帧视频`(AddGuide 锚定任意帧)/ `4030-参考视频` |
