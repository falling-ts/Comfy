...
1. **节点间距与不重叠**
   - 节点之间**严禁重叠**;
   - 相邻节点(横向或纵向相邻)之间边距 **10~80px**;
   - 如果有节点有多条连出去的线，当满足单个与之相邻节点连线的距离后，其它与之相连的节点不受边距限制，但是所有节点之间不得重叠；
   - 对角线相隔较远的不算相邻, 不受上限限制;
   - **`FallingTSMarkDownTable`(MD 数据表)节点在画布中单独占一列**: 其所在列的整条垂直方向(正下方无限延伸, 无像素限制)严禁放置任何节点——MD 节点会根据数据内容自动向下扩展高度, 同列下方有节点会被覆盖; 其他节点只能放在 MD 节点右侧的其他列(列间满足 10~80px 边距), 与 MD 节点同列的任何位置均不得放置节点。
