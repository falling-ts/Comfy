...
**固定的视频事件框架 + 动态的用户风格填充 + 锁定的角色身份 + 色彩联动 UI 系统。**

视频提示词不得盲目保留源提示词的默认风格。它必须保留时间线、UI 事件、玩家位置、装备逻辑和文字结构,同时根据用户选择的风格动态改写视觉处理、调色板、角色渲染、光照、UI 表面、图标和城市世界风格。

## 优先级顺序
1. 用户选择的风格:{visual_style}
2. 确认图 / UI 参考:{ui_ref}
...
## 参考图作用
- {ui_ref}:已确认的第一张图。用它锁定 UI 布局、菜单层级、色彩系统、字号比例、按钮结构、角色与游戏界面融合以及整体构图逻辑。
- {player1_ref}:PLAYER 1 身份锚点。锁定确切的脸、发型、眼镜(如有)、面部比例、身体身份以及与 {player1_name} 的昵称对应关系。
- {player2_ref}:PLAYER 2 身份锚点。锁定确切的脸、发型、面部比例、身体身份以及与 {player2_name} 的昵称对应关系。
- {height_ref}:体型对比锚点。锁定两名玩家之间可见的对比,防止两人体型比例趋同。
...
## 负面约束
禁止第三人、除非用户明确要求否则禁止女性角色、禁止重复角色、禁止角色交换、禁止昵称交换、禁止身体融合、禁止体型比例趋同、禁止改变脸、禁止改变发型、禁止改变身份、禁止缺少 PLAYER 1、禁止缺少 PLAYER 2、禁止分屏、禁止硬切、禁止随机镜头抖动、禁止漂浮的身体部位、禁止血腥断肢、禁止武器、除非用户风格选择否则禁止过度霓虹、除非用户风格选择否则禁止紫色背景、禁止不可读 UI、禁止随机字母、禁止拼写错误的用户名、禁止多余菜单选项、禁止官方游戏标志、禁止复制的品牌界面、禁止水印。
