...
### 图片/万物类(001x,3 个,基于 Qwen-Image-2512 / Qwen-Edit 2511 / FLUX.2-Klein)

| 工作流 | 用途 |
|--------|------|
| `0011-万物建模` | 主线主流程(万物建模) |
| `0010-灰度遮罩` | 灰度遮罩工具 |
| `0012-万物变化` | 万物变化/变换 |
...
### 视频生成类(003x-004x,9 个,MiniMax H3)

| 工作流 | 用途 | H3 模式 |
|--------|------|---------|
| `0030-文生场景` | 文本 → 场景视频(仅画面,无音轨) | T2VA(fl2va) |
| `0031-首帧场景` | 首帧图 → 场景视频 | I2V(fl2va) |
| `0032-参考场景` | 多图 + 多视频参考 → 视频 | R2V(ref2va) |
| `0033-OrbitSheets场景` | 锚点图 → H3 多视角运镜 → 视觉选帧拼「场景参考板」网格图 | I2V(fl2va)+ OrbitSheets 插件 |
| `0040-文生视频` | 文本 → 视频(与 0030 同构的通用版) | T2VA(fl2va) |
| `0041-首帧视频` | 首帧图 → 视频 | I2V(fl2va) |
| `0042-首尾视频` | 首尾帧 → 视频 | fl2va |
| `0044-参考视频` | 参考图/视频 → 视频 | R2V(ref2va) |

> 0030/0031/0032 与 0040/0041/0042/0044 结构一一对应(前者为场景流水线版,后者为通用版);均已去除音频轨道,视频 + 音频在后期流水线(005x-007x)中合并。0033 为场景参考生产补充:产出「场景参考板」网格图(供 0032/0044 作参考输入)。
...
| 工作流 | 用途 | 核心模型 |
|--------|------|---------|
| `0060-背景音乐` | 纯器乐 BGM | Stable Audio 3(内置 Sage 加速) |
| `0061-环境音效` | 环境/氛围音 | Stable Audio 3 |
| `0062-效果音效` | 一次性/打击音效 | Stable Audio 3 |
| `0063-文生人声` | 文本描述音色 → 说话 | Qwen3-TTS VoiceDesign |
| `0064-参考人声` | 3s 参考音频克隆 → 说话 | Qwen3-TTS CustomVoice |
| `0070-截取声音` | 音频裁剪/截取工具 | — |
...
> **fl2va 与 ref2va 是同一基座的任务变体**:文生/图生工作流(0030/0031/0040/0041)用 fl2va,参考生成工作流(0032/0044)用 ref2va,两者都要下。仓库另有 bf16/int8_convrot/pruned_fp8_scaled 档可选。
